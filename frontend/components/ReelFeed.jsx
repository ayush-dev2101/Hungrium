...
import {Link} from 'react-router-dom' //We are going to import the Links in the feed 
...
const ReelFeed = ({items = [], onLike, onSave, emptyMessage = "No videos yet."}) => {
  ...
  const setVideoRef = (id) => (e) =>{
    if(!e){videoRefs.current.delete(id); 
      return}
    videoRefs.current.set(id, e)
  }
  return(
      ...
      <div className='reels-feed' role='list'>
        ...
        {items.map((item)=>(
          <section key={item._id} className='reel' role='listitem'>
            <video src={item.video} ref={setVideoRef(item._id)} muted playsInline loop preload='metadata'></video>
            <div className='reel-overlay'>
              ...
              <div className='reel-actions'>
                ...
                <div className='reel-action-group' 
                >
                  <button className='reel-action' onClick={onSave ? () => onSave(item): undefined} aria-label='Bookmark'>
                  ...
                  <div className='reel-action-group'>
                    ...
                    <div className='reel-action__count'>{item.commentCount ?? (Array.isArray(item.comments)? item.comments.length: 0)}</div>
                  </div>
